		...
		setTimeout(() => {
			...
			for (var i = 0; i < elements.length; i++) {
				elements[i].addEventListener('click',()=>{
					...
					this.letInform.get('message')?.setValidators(Validators.required)
					this.letInform.get('message')?.updateValueAndValidity()
				});
			}
			...
			for (var i = 0; i < elements.length; i++) {
				elements[i].addEventListener('click',()=>{
					...
					this.letInform.get('message')?.clearValidators()
					this.letInform.get('message')?.updateValueAndValidity()
				});
			}
		}, 100);
